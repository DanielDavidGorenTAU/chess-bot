...
ROTATED_DOWN_ORIENTATION = [-0.8842933603715832, -2.9824891164041816, 0.006334514932163864] 
...
def calibrate_board_positions(a1=A1, h8=H8):
    ...
    grip_height["knight"] = FLOOR_HEIGHT + 0.03
    grip_height["bishop"] = FLOOR_HEIGHT + 0.028
...
class ChessBot:
    ...
    def mov_chess_piece(self, type=None, start_pos=None, end_pos=None):
        ...
        self.move_to(z=grip_height[type] + 0.002)
    ...
    def mov_chess_piece_rotated(self, type=None, start_pos=None, end_pos=None):
        if self.pose[Z] < SAFE_HEIGHT:
            self.move_to(z=SAFE_HEIGHT)
        ...
        self.move_to(start_pos, z=SAFE_HEIGHT)
        self.move_to(rx=ROTATED_DOWN_ORIENTATION[0], ry=ROTATED_DOWN_ORIENTATION[1], rz=ROTATED_DOWN_ORIENTATION[2])
        self.move_to(z=grip_height[type])

        self.set_gripper(grip_size[type]) # grip the piece

        self.move_to(start_pos[0:3] + ROTATED_DOWN_ORIENTATION, z=SAFE_HEIGHT)
        self.move_to(end_pos[0:3] + ROTATED_DOWN_ORIENTATION, z=SAFE_HEIGHT)
...
if __name__ == "__main__":
    with ChessBot() as bot:
        ...
        bot.mov_chess_piece('pawn', positions["f8"], positions["f3"])
        #bot.set_gripper(255)
        #bot.move_to(rx=ROTATED_DOWN_ORIENTATION[0], ry=ROTATED_DOWN_ORIENTATION[1], rz=ROTATED_DOWN_ORIENTATION[2])
        #tmp = bot.rtde_r.getActualTCPPose().copy()
        #print(tmp)
        #tmp[RZ] -= 0.3
        #print(tmp)
        #bot.rtde_c.moveJ(tmp, 0.1, 0.1)
        #print(bot.pose)
        #print(DOWN_ORIENTATION)


        # [-0.08752209134752272, -0.5269607544176779, 0.3264302683205411, 0.03513221627181906, -3.113177631839201, 0.020226266839726507]
        # [-0.08753678097976621, -0.5269631916848093, 0.32640222570112043, -0.8842933603715832, -2.9824891164041816, 0.006334514932163864]
